...
# ---------------------------------------
# Fonction de calcul de l'évolution journalière (format simplifié)
# ---------------------------------------
def compute_daily_evolution(
    ...
    """
    Calcule l'évolution journalière des quantités vendues avec filtres optionnels
    
    Returns:
        Liste de dictionnaires avec format: [{"date": "YYYY-MM-DD", "quantity": X}]
    """
    conn = None
    cur = None
    try:
        ...
        # 🔹 Requête simplifiée pour l'évolution journalière
        query = f'''
            SELECT 
                DATE(p."createdAt") as sale_date,
                SUM(b.quantity) as daily_quantity
        ...
        print(f" REQUÊTE ÉVOLUTION JOURNALIÈRE SIMPLIFIÉE: {query}")
        ...
        print(f" RÉSULTAT SIMPLIFIÉ: {len(result)} jours de données")

        return result

    except Exception as e:
        print(f" Erreur dans compute_daily_evolution: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Erreur lors du calcul de l'évolution journalière: {str(e)}")
...
# ---------------------------------------
# Endpoint FastAPI - Évolution journalière (format simplifié)
# ---------------------------------------
@router.get("/daily_evolution")
def daily_evolution(
    ...
    """
    Retourne l'évolution journalière des quantités vendues
    ...
    try:
        ...
        if cached:
            print("⚡ HIT CACHE - daily_evolution")
            return json.loads(cached)

        # 🔹 Calculer si cache miss
        print("🔄 CALCUL DIRECT - daily_evolution")
        result = compute_daily_evolution(
            date_start=date_start,
            date_end=date_end,
            events=events,
            companies=companies,
        ...
        print(f"❌ Erreur inattendue dans daily_evolution: {str(e)}")
